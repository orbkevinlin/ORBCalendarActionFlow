...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _titleColors[@(FSCalendarCellStateExpiredSelected)] = [UIColor whiteColor];
...
- (void)setExpirationColor:(UIColor *)color {
    if (color) {
        _titleColors[@(FSCalendarCellStateExpired)] = color;
    } else {
        [_titleColors removeObjectForKey:@(FSCalendarCellStateExpired)];
    }
    [self invalidateTitleTextColor];
}

- (UIColor *)expirationColor {
    return _titleColors[@(FSCalendarCellStateExpired)];
}
...
- (void)setSelectExpirationColor:(UIColor *)color {
    if (color) {
        _backgroundColors[@(FSCalendarCellStateExpiredSelected)] = color;
    }else{
        [_backgroundColors removeObjectForKey:@(FSCalendarCellStateExpiredSelected)];
    }
    [self invalidateFillColors];
}

- (UIColor *)selectExpirationColor {
    return _backgroundColors[@(FSCalendarCellStateExpiredSelected)];
}
